...
public struct AnimatorTransitionInfo
{
    ...
    {
        get { return m_HasFixedDuration ? DurationUnit.Fixed : DurationUnit.Normalized; }
    }
    ...
    [NativeName("fullPathHash")] private int m_FullPath;
    [NativeName("userNameHash")] private int m_UserName;
    [NativeName("nameHash")] private int m_Name;
    [NativeName("hasFixedDuration")] private bool m_HasFixedDuration;
    [NativeName("duration")] private float m_Duration;
    [NativeName("normalizedTime")] private float m_NormalizedTime;
    [NativeName("anyState")] private bool m_AnyState;
    [NativeName("transitionType")] private int m_TransitionType;
}
...
{
    // Utu Animation Play Timer
    private float animPlayTime { get; set; }
    private float animEndTime { get; set; } = -1;
    ...
    extern public bool applyRootMotion { get; set; }
    ...
    extern public bool linearVelocityBlending { get; set; }
    ...
    extern public AnimatorUpdateMode updateMode { get; set; }

    // Tell if the corresponding Character has transform hierarchy.
    extern public bool hasTransformHierarchy { get; }

    extern internal bool allowConstantClipSamplingOptimization { get; set; }
    ...
    public void SetLookAtWeight(float weight, [DefaultValue("0.0f")] float bodyWeight,
        [DefaultValue("1.0f")] float headWeight, [DefaultValue("0.0f")] float eyesWeight,
    ...
    extern private void SetLookAtWeightInternal(float weight, float bodyWeight, float headWeight, float eyesWeight,
        float clampWeight);
    ...
    {
        return InternalGetBehavioursByKey(fullPathHash, layerIndex, typeof(StateMachineBehaviour)) as
            StateMachineBehaviour[];
    }

    [FreeFunction(Name = "AnimatorBindings::InternalGetBehavioursByKey", HasExplicitThis = true)]
    extern internal ScriptableObject[] InternalGetBehavioursByKey(int fullPathHash, int layerIndex,
        [NotNull] Type type);

    // Automatic stabilization of feet during transition and blending
    extern public bool stabilizeFeet { get; set; }
    ...
    extern private void GetAnimatorStateInfo(int layerIndex, StateInfoIndex stateInfoIndex,
    ...
    public AnimatorStateInfo GetCurrentAnimatorStateInfo(int layerIndex)
    {
        ...
        info.normalizedTime = (UGUSDAnimatorBaseUtil.GetCurrentTime(owner) - animPlayTime) / animEndTime;
        return info;
    }
    ...
    extern public float feetPivotActive { get; set; }
    ...
    extern private void MatchTarget(Vector3 matchPosition, Quaternion matchRotation, int targetBodyPart,
        MatchTargetWeightMask weightMask, float startNormalizedTime, float targetNormalizedTime,
        bool completeMatch);
    ...
    public void MatchTarget(Vector3 matchPosition, Quaternion matchRotation, AvatarTarget targetBodyPart,
        MatchTargetWeightMask weightMask, float startNormalizedTime)
    {
        MatchTarget(matchPosition, matchRotation, (int)targetBodyPart, weightMask, startNormalizedTime, 1, true);
    }

    public void MatchTarget(Vector3 matchPosition, Quaternion matchRotation, AvatarTarget targetBodyPart,
        MatchTargetWeightMask weightMask, float startNormalizedTime, [DefaultValue("1")] float targetNormalizedTime)
    {
        MatchTarget(matchPosition, matchRotation, (int)targetBodyPart, weightMask, startNormalizedTime,
            targetNormalizedTime, true);
    }
    ...
    extern public bool isMatchingTarget { [NativeMethod("IsMatchingTarget")] get; }
    ...
    public void CrossFadeInFixedTime(string stateName, float fixedTransitionDuration)
    {
        ...
        int layer = -1;
        CrossFadeInFixedTime(StringToHash(stateName), fixedTransitionDuration, layer, fixedTimeOffset,
    ...
    public void CrossFadeInFixedTime(string stateName, float fixedTransitionDuration,
        [DefaultValue("-1")] int layer, [DefaultValue("0.0f")] float fixedTimeOffset,
    ...
    extern public void CrossFadeInFixedTime(int stateHashName, float fixedTransitionDuration,
        [DefaultValue("-1")] int layer, [DefaultValue("0.0f")] float fixedTimeOffset,
        [DefaultValue("0.0f")] float normalizedTransitionTime);
    ...
    public void CrossFade(string stateName, float normalizedTransitionDuration, [DefaultValue("-1")] int layer,
    ...
    extern public void CrossFade(int stateHashName, float normalizedTransitionDuration,
        [DefaultValue("-1")] int layer, [DefaultValue("0.0f")] float normalizedTimeOffset,
        [DefaultValue("0.0f")] float normalizedTransitionTime);
    ...
    public void PlayInFixedTime(string stateName, [DefaultValue("-1")] int layer,
    ...
    extern public void PlayInFixedTime(int stateNameHash, [DefaultValue("-1")] int layer,
        [DefaultValue("float.NegativeInfinity")] float fixedTime);
    ...
    {
        AActor curActor = u1Component.GetOwner();
        TArray<FName> TagsList = curActor.Tags;
        bool bIsUiAnim = false;
        foreach (FName tag in TagsList)
        {
            string str = tag.ToString();
            if (str.Contains(UiCommon.ui_tag))
            {
                bIsUiAnim = true;
                break;
            }
        ...
        if (bIsUiAnim)
        {
            ...
            FString curPath = UGUSDUiUtil.GetInstance().GetAttachActorPath("", ref curActor);
            UUserWidget userWidget = UGUSDUiUtil.GetInstance().FindUserWidget(curPath, ref curActor,ref isWorldSpace);
            // TODO: 条件2实际上限制了在组件生命周期内，Play只能执行一次
            // 这并不合理，但却符合AB的表现（AB中Play方法被调用了两次，但第二次调用后未产生实际的动画播放行为）
            if (userWidget != null && animEndTime == -1 )
            {
                UUMGSequencePlayer Player = UGUSDAnimatorBaseUtil.PlayUiAnim(u1Component.GetOwner(), stateName, userWidget);
                animEndTime = Player.Animation.GetEndTime();
                animPlayTime = UGUSDAnimatorBaseUtil.GetCurrentTime(u1Component.GetOwner());
            }
            return;
        }
        float normalizedTime = float.NegativeInfinity;
    ...
    extern public void Play(int stateNameHash, [DefaultValue("-1")] int layer,
        [DefaultValue("float.NegativeInfinity")] float normalizedTime);
    ...
    extern public Transform avatarRoot { get; }
    ...
    extern public AnimatorCullingMode cullingMode { get; set; }
    ...
    // Plays recorded data
    extern public float playbackTime { get; set; }
    ...
    extern public RuntimeAnimatorController runtimeAnimatorController { get; set; }

    // Returns true if Animator has any playables assigned to it.
    extern public bool hasBoundPlayables { [NativeMethod("HasBoundPlayables")] get; }

    extern internal void ClearInternalControllerPlayable();

    extern public bool HasState(int layerIndex, int stateID);

    private static Dictionary<int, string> hashToParamName = new();
    ...
    public static int StringToHash(string name)
    {
        ...
        hashToParamName.TryAdd(hashCode, name);
        return hashCode;
    }

    // Gets/Sets the current Avatar
    extern public Avatar avatar { get; set; }
    ...
    {
        FName Name = new FName(name);
        UGUSDAnimatorBaseUtil.SetFloat(u1Component.GetOwner(), Name, value);
    }
    ...
    {
        string name;
        hashToParamName.TryGetValue(id, out name);
        SetFloat(name, value);
    }
    ...
    {
        FName ParamName = new FName(name);
        return (float)UGUSDAnimatorBaseUtil.GetFloat(u1Component.GetOwner(), ParamName);
    }
    ...
    {
        FName Name = new FName(name);
        UGUSDAnimatorBaseUtil.SetBool(u1Component.GetOwner(), Name, value);
    }
    ...
    {
        FName ParamName = new FName(name);
        return UGUSDAnimatorBaseUtil.GetBool(u1Component.GetOwner(), ParamName);
    }
    ...
    {
        FName Name = new FName(name);
        UGUSDAnimatorBaseUtil.SetInteger(u1Component.GetOwner(), Name, value);
    }
    ...
    {
        FName ParamName = new FName(name);
        return UGUSDAnimatorBaseUtil.GetInteger(u1Component.GetOwner(), ParamName);
    }
    ...
    extern public bool layersAffectMassCenter { get; set; }
    ...
    extern public bool logWarnings { get; set; }
    extern public bool fireEvents { get; set; }

    [Obsolete(
        "keepAnimatorControllerStateOnDisable is deprecated, use keepAnimatorStateOnDisable instead. (UnityUpgradable) -> keepAnimatorStateOnDisable",
        false)]
    ...
    extern public bool keepAnimatorStateOnDisable { get; set; }

    extern public bool writeDefaultValuesOnDisable { get; set; }
