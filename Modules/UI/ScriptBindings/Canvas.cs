...
{
    public UCanvasPanel canvasWidget { get; set; }
    ...
    void Awake()
    {
        var compName = u1Component.GetName();
        Debug.LogDebug("CanvasU3_C Awake start, name = " + compName);
        if (canvasWidget == null)
        {
            var curActor = owner;
            var curActorOwner = u1Component.GetOwner();
            bool isWorldSpace = false;
            FString curPath = UGUSDUiUtil.GetInstance().GetAttachActorPath("", ref curActor);
            var targetWidget = UGUSDUiUtil.GetInstance().FindTargetWidget(curPath, ref curActorOwner, ref isWorldSpace);
            if (targetWidget != null && targetWidget is UCanvasPanel panel)
            {
                canvasWidget = panel;
                Debug.LogDebug("CanvasU3_C Awake, targetWidget is not null, comp name = " + compName +
                               ", curPath = " + curPath + ", targetWidget name = " + targetWidget.GetName());
            }
            else
            {
                Debug.LogDebug("CanvasU3_C Awake, targetWidget is null, name = " + compName + ", curPath = " + curPath);
            }
        }
    }
    ...
    public override bool enabled
    {
        get
        {
            if (canvasWidget == null)
                return false;
            var visibility = canvasWidget.GetVisibility();
            return visibility is ESlateVisibility.Visible or ESlateVisibility.HitTestInvisible or ESlateVisibility.SelfHitTestInvisible;
        }
        set
        {
            if (canvasWidget == null)
            ...
            var visibility = value ? ESlateVisibility.SelfHitTestInvisible : ESlateVisibility.Hidden;
            canvasWidget.SetVisibility(visibility);
            base.enabled = value;
        }
    }
    ...
    public float scaleFactor { get; set; } = (float)1.0;
